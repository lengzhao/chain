	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if deserialized.Type != tc.request.Type {
				t.Errorf("Type 不匹配: 期望 %s, 实际 %s", tc.request.Type, deserialized.Type)
			}

			if len(deserialized.Data) != len(tc.request.Data) {
				t.Errorf("Data 长度不匹配: 期望 %d, 实际 %d", len(tc.request.Data), len(deserialized.Data))
			}

			if len(tc.request.Data) > 0 {
				for i := range tc.request.Data {
					if deserialized.Data[i] != tc.request.Data[i] {
						t.Errorf("Data 内容不匹配: 位置 %d, 期望 %d, 实际 %d", i, tc.request.Data[i], deserialized.Data[i])
						break
					}
				}
			}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if deserialized.Type != tc.response.Type {
				t.Errorf("Type 不匹配: 期望 %s, 实际 %s", tc.response.Type, deserialized.Type)
			}

			if len(deserialized.Data) != len(tc.response.Data) {
				t.Errorf("Data 长度不匹配: 期望 %d, 实际 %d", len(tc.response.Data), len(deserialized.Data))
			}

			if len(tc.response.Data) > 0 {
				for i := range tc.response.Data {
					if deserialized.Data[i] != tc.response.Data[i] {
						t.Errorf("Data 内容不匹配: 位置 %d, 期望 %d, 实际 %d", i, tc.response.Data[i], deserialized.Data[i])
						break
					}
				}
			}
...
func TestBinaryFormatEfficiency(t *testing.T) {
	...
	if len(serialized) != expectedSize {
		t.Errorf("序列化大小不匹配: 期望 %d, 实际 %d", expectedSize, len(serialized))
	}
